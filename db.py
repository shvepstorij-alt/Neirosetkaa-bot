...
async def init_db():
    ...
    async with pool.acquire() as conn:
        ...
        for col, dfn in [
            ...
            ("gpt_email",      "TEXT"),
            # Organization ID аккаунта ChatGPT. У тарифа Go (маршрут iOS) сайт
            # привязывает код именно к нему, а колонка «Почта» у него пустая —
            # сверять такие активации по почте нечем в принципе.
            ("gpt_org",        "TEXT"),
            # Кандидаты в Organization ID из начала присланного текста, через
