...
FREE_CREDITS   = 100  # кредитов при первом /start без рефералки
...
REF_BONUS = 200  # кредитов ПРИГЛАСИВШЕМУ за реферала (после первой покупки друга)
# Кредиты ПРИГЛАШЁННОМУ при регистрации по ссылке. Отдельная константа: раньше
# обе выплаты шли из REF_BONUS, поэтому изменить одну без другой было нельзя.
REF_WELCOME_CREDITS = 150
...
def friendly_error(e: Exception) -> str:
    ...
    # Провайдер прислал ПОЛЕЗНОЕ объяснение (нестабильность модели, список
    # альтернатив, ошибка параметров) — показываем его как есть. Раньше такие
    # тексты подменялись общим «небольшая техническая проблемка», и клиент
    # оставался без подсказки, что делать дальше.
    if ("нестабил" in low or "временно недоступна" in low
            or "попробуй альтернативу" in low or "выбери другую модель" in low
            or "ошибка параметров" in low or "завис в очереди" in low
            or "таймаут генерации" in low):
        return err
    # Модель вернула текст вместо картинки (клиент прислал разговорный промт/инструкцию)
    if "не смогла создать картинку" in low or "пустой ответ" in low or "попробуй другой промт" in low:
        return (
            "⚠️ <b>Похоже, это текст-инструкция, а не описание картинки.</b>\n\n"
            "Опиши, ЧТО показать на изображении: объект, стиль, фон, освещение.\n"
            "Например: <i>рыжий кот в очках сидит в кафе, тёплый свет, реализм</i>"
        )
...
При регистрации — 100 кредитов бесплатно (150 по реферальной ссылке). Оплата СБП.
...
━━━━━━━━━━━━━━━━━━━━
📢 Новости и гайды: @{channel}
Выбери действие 👇"""

# Монетки в приветствии показываем только тем, у кого они есть: строка
# «🪙 Монетки: 0 ₽» ничего не сообщает большинству, а вот человеку с балансом
# важно про них помнить. 18.09.2026 клиент решил, что монетки «куда-то
# пропали», — просто потому, что нигде их не видел и не знал, что с ними
# делать. Поэтому рядом с суммой сразу сказано, куда их девать.
WELCOME_BACK = """👋 С возвращением, {name}!

💵 Баланс: <b>{credits} кр</b>{coins_line}
