...
async def admin_gpt_check_pool(message: Message):
    """Сверяет пулы ChatGPT, Claude и Perplexity с сайтом активации.

    То же, что делает фоновая задача раз в 3 часа. Все коды уходят одним
    запросом — сайт различает их по префиксу.
    """
    if not is_admin(message.from_user.id):
        return
