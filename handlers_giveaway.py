...
async def giveaway_where(message: Message):
    ...
    try:
        ...
        _st = str(getattr(getattr(_m, "status", ""), "value",
                          getattr(_m, "status", ""))).split(".")[-1].lower()
        _me = _st or "?"
    except Exception as _e:
        _me = f"не вижу ({_e})"
    # Главное, чего не видно снаружи: обычный участник группы НЕ получает чужие
    # сообщения — у ботов включён режим приватности, и им приходят только
    # команды. Администратор получает всё. То есть бот-«member» будет молча
    # пропускать каждое «Участвую», и выглядеть это будет как «никто не пришёл».
    _adm = _st in ("administrator", "creator", "owner")
    _tail = ("✅ Бот администратор — видит все комментарии."
             if _adm else
             "🚨 <b>БОТ НЕ АДМИНИСТРАТОР — КОММЕНТАРИИ НЕ СОБИРАЮТСЯ.</b>\n"
             "У ботов включён режим приватности: обычному участнику группы "
             "чужие сообщения просто не приходят. Дай боту права "
             "администратора в группе обсуждения (достаточно любых) — "
             "и он начнёт видеть «Участвую».")
    await message.answer(
